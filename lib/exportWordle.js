import { PHONEME_LABELS, KEYBOARD_ROWS } from "./phonemes";
...
export function generateWordleHTML({ target, maxGuesses, activityTitle }) {
  const data = {
    word: target.word,
...
<body>
  ...
  <p class="sub">Guess the phoneme-based word. Select a phoneme for each box, then press Enter. Hover or focus a key to see its English letter equivalent.</p>
    ...
    groups.forEach(group => {
      ...
      group.rows.forEach(row => {
        ...
        row.forEach(ipa => {
          ...
          key.textContent = ipa;
          const m = meta(ipa);
          key.setAttribute('aria-label', '/' + ipa + '/ sounds like ' + m.label + ' as in ' + m.example);
