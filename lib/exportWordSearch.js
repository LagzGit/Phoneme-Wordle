...
export function generateWordSearchHTML({
  ...
  const data = { grid, placements, words, showHints, labels: PHONEME_LABELS };
...
<body>
  ...
  <p class="sub">${esc(instructions || "Find every phoneme-based word hidden in the grid. Select the first and last phoneme of each word.")}</p>
  ...
  function renderWordList() {
    ...
    DATA.words.forEach(w => {
      ...
      const chips = w.phonemes.map(ipa => {
        ...
        const label = DATA.showHints
          ? '/' + ipa + '/ sounds like ' + meta.label + ', as in ' + meta.example
          : 'Phoneme /' + ipa + '/';
        const hint = DATA.showHints
          ? '<span class="hint">' + escapeHTML(meta.label) + ' (as in ' + escapeHTML(meta.example) + ')</span>'
          : '';
        return '<span class="phoneme-chip" tabindex="0" aria-label="' + escapeHTML(label) + '">/' + escapeHTML(ipa) + '/' + hint + '</span>';
      }).join(' ');
      const wordHint = DATA.showHints && w.hint
        ? '<small style="flex-basis:100%;color:var(--ink-soft);">' + escapeHTML(w.hint) + '</small>'
        : '';
      item.innerHTML = chips + ' <span style="margin-left:auto;font-weight:700;">' + escapeHTML(w.word) + '</span>' + wordHint;
