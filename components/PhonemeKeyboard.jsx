...
export default function PhonemeKeyboard({ onSelect, disabled = false }) {
...
function KeyGroup({ label, rows, onSelect, disabled, className = "" }) {
  return (
      ...
      <div className="space-y-1">
        {rows.map((row, ri) => (
          <div key={ri} className="flex justify-center gap-1">
            {row.map((ipa, ci) =>
              ipa ? (
                <Key key={ci} ipa={ipa} onSelect={onSelect} disabled={disabled} />
              ) : (
...
function Key({ ipa, onSelect, disabled }) {
  ...
  return (
    <button
      ...
      aria-label={`/${ipa}/ sounds like ${meta?.label ?? ipa}, as in ${meta?.example ?? ""}`}
    >
      {ipa}
      {meta && (
